...
int Write(uint32_t address, uint32_t size, uint8_t* buffer){
	...
	while (size > CONFIG_EEPROM_PAGE_SIZE) {
		WritePage(address, CONFIG_EEPROM_PAGE_SIZE, buffer);
		buffer  += CONFIG_EEPROM_PAGE_SIZE;
		address += CONFIG_EEPROM_PAGE_SIZE;
		size    -= CONFIG_EEPROM_PAGE_SIZE;

		for (volatile uint32_t i = 10000; i; i--);
	}

	if (size) WritePage(address, size, buffer);
